...
cbow 모델과 skip-gram 모델을 활용하여 단어 임베딩을 학습하고, PCA를 통해 시각화하여 단어 간 유사도를 알아보는 실습입니다. 
시각화 그래프를 통해 두 모델 간의 학습 방식 차이를 비교해봅니다.

## 📁 파일
📄[`NLP_word_embedding_mission.ipynb`](./NLP_word_embedding_mission.ipynb)
→ 데이터셋: [`fairy_tale.txt`](./fairy_tale.txt)

## 🧪 실습 주제

- 데이터 준비 및 전처리, Okt 형태소 분석기를 이용해 형태소 단위로 토큰화
- Word2Vec 모델 학습(CBOW, Skip-gram 방식)
- 모델 비교 및 유사도 분석
- 임베딩 시각화(PCA)
- 결과 해석

## 📚 학습 소감
해당 데이터셋으로 유사도 분석 및 시각화한 결과만으로 본 동화 코퍼스에서는 CBOW 방식과 Skip-gram방식 중 어떤 것이 더 적합한지 판단하기 어려웠다.
데이터 규모가 작아서 그런 것 같다. 이번 실습에서는 cbow, skip-gram 모델 활용하여 단어 임베딩을 시각화하고, 유사도를 비교해본 경험 자체에 의미를 두고자 한다.
다음에는 더 큰 규모의 데이터셋으로 모델 성능을 비교 분석해보며, 두 임베딩 방식의 차이를 더 뚜렷하게 관찰해봐야겠다.
